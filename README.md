...
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ---------------------------- |
| `SECRET_KEY`                         | **CRITICAL:** A strong, random key for session security and data encryption.                               | Any strong random string (ex. 32 hex characters)   | (none - **must be set**)     |
| `DEPLOYMENT_MODE`                    | Application deployment mode. `multi` enables user accounts and individual API keys.                        | `single`, `multi`                                    | `multi`                      |
| `TZ`                                 | The timezone for the application.                                                                          | Any valid timezone string (ex. `UTC`, `Europe/Paris`) | `UTC`                        |
...
| `SUPPORTED_LANGUAGE_CODES`           | Comma-separated list of language codes to show in the UI.                                                  | ex. `en,nl,fr,es`                                  | `en,nl,fr,es`                |
| `MYSQL_HOST`                         | Hostname for the MySQL database server.                                                                    | ex. `mysql` (service name in Docker), `localhost`  | `localhost`                  |
| `MYSQL_PORT`                         | Port for the MySQL database server.                                                                        | ex. `3306`                                         | `3306`                       |
...
| `MYSQL_POOL_SIZE`                    | Number of connections in the MySQL connection pool.                                                        | Integer (ex. `10`)                                 | `10`                         |
| `ADMIN_USERNAME`                     | Username for the initial administrator account (created on first run in `multi` mode).                     | ex. `admin`                                        | `admin`                      |
...
| `MAIL_SERVER`                        | SMTP server for sending emails (ex. password resets).                                                    | ex. `smtp.gmail.com`                               | (none)                       |
| `MAIL_PORT`                          | SMTP server port.                                                                                          | ex. `587` (TLS), `465` (SSL)                       | `587`                        |
...
| `MAIL_DEFAULT_SENDER`                | Default sender email address for application emails.                                                       | ex. `noreply@example.com`                          | `noreply@example.com`        |
...
| `WORKFLOW_LLM_PROVIDER`              | Default LLM provider for workflow tasks.                                                                   | ex. `gemini-2.0-flash`, `gpt-4o`                   | `gemini-2.0-flash`           |
| `WORKFLOW_MAX_OUTPUT_TOKENS`         | Maximum tokens for workflow LLM responses.                                                                 | Integer (ex. `1024`)                               | `1024`                       |
| `WORKFLOW_RATE_LIMIT`                | Rate limit for workflow API calls per user.                                                                | ex. `10 per hour`                                  | `10 per hour`                |
| `DELETE_THRESHOLD`                   | Age in seconds after which temporary uploaded audio files are deleted.                                     | Integer (ex. `86400` for 24h)                      | `86400`                      |
| `PHYSICAL_DELETION_DAYS`             | Days after a transcription record is soft-deleted before it's physically removed from the database.        | Integer (ex. `120`)                                | `120`                        |
| `LOG_LEVEL`                          | Application logging level.                                                                                 | `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`      | `INFO`                       |
| `TRANSCRIPTION_WORKERS`              | Number of parallel workers for chunked transcription (OpenAI models). AssemblyAI uses 1.                   | Positive Integer (ex. `4`)                         | `4`                          |
...
If a pre-built Docker image is available on Docker Hub (ex. `arnoulddw/transcriber-platform:latest`).
...
4.  **Set up MySQL Database:**
    Ensure you have a MySQL server running and accessible. Create a database (ex. `transcriber_db`) and a user with permissions to access it.
...
5.  **Add Translations (Manual Step):**
    Edit the `.po` files (ex. `app/translations/es/LC_MESSAGES/messages.po`) and fill in the translations for any entries with an empty `msgstr ""`.
...
-   **Port `5004` (or configured `APP_PORT`) is in use:**
    If using Docker Compose, change the host port in the `ports` section of `docker-compose.yml` (ex. `"5005:5004"`) or update `APP_PORT` in your `.env` file and rebuild/restart. If running locally, use `flask run --port <new_port>`.
...
-   **File Upload Issues:**
    Verify that your audio file is in a supported format (ex. mp3, m4a, wav, webm) and not corrupted. Check browser console for errors.
-   **Google Sign-In Issues (Multi-Mode):**
    -   Ensure `GOOGLE_CLIENT_ID` is correctly set in `.env`.
    -   Verify that your Google Cloud Project's OAuth 2.0 credentials have the correct Authorized JavaScript origins (ex. `http://localhost:5004`) and Authorized redirect URIs (ex. `http://localhost:5004/api/auth/google-callback`).
